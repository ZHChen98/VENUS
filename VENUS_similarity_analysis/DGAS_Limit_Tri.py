...
class DGAS_limit_core:
    ...
    def start(self,gene1,gene2,gene3,max_length):
        """
        :param gene1:
        :param gene2:
        ...
        #     return []
        meta_paths, meta_paths_from_gene1, meta_path_from_gene2, meta_path_from_gene3=self.find_meta_paths(gene1,gene2,gene3,graph,max_step)
        if len(meta_paths)==0:
            return []
        meta_paths_list=[]
        for i in meta_paths:
        ...
        score_list=self.score(meta_paths,meta_paths_from_gene1,meta_path_from_gene2,meta_path_from_gene3,meta_paths_combine_list)
        for i in score_list:
            i['gene1']=gene1
            i['gene2']=gene2
            i['gene3']=gene3
    ...
    def find_meta_paths(self,gene1,gene2,gene3,graph,max_step):
        """
        ...
        tmPaths = self.map2
        self.find_meta_paths_g3_center(gene3, graph, center, self.get_node_type(gene3), gene3)
        xmPaths = self.map3
        mPaths = self.match_meta_path(hmPaths, tmPaths, xmPaths)


        return mPaths,hmPaths,tmPaths,xmPaths

    def match_meta_path(self, head_map, tail_map, third_map):
        meta_path_list = []
        for i, k in head_map.items():
            # print(i)
            if tail_map.__contains__(i) and third_map.__contains__(i):
                meta_path_name = i + i[0:-1][::-1]
                meta_count = 0
                found1 = tail_map[i]
                found2 = third_map[i]
                for node, count in k.items():
                    if found1.__contains__(node) and found2.__contains__(node):
                        count2 = found1[node]['ins']
                        count3 = found2[node]['ins']
                        meta_count += count['ins'] * count2 * count3

                        if len(i) > 3:
                            check_len = len(i) - 3
                            list1 = count['list']
                            list2 = found1[node]['list']
                            list3 = found2[node]['list']
                            check_nodes = []
                            check_nodes2 = []
                            check_nodes3 = []
                            for path1 in list1:
                            ...
                            for path3 in list3:
                                nodes3 = path3.split('|')
                                check_nodes3.append(nodes3[1:1 + check_len])
                            for c1 in check_nodes:
                                for c2 in check_nodes2:
                                    for c3 in check_nodes3:
    ...
    def find_meta_paths_g3_center(self, gene, graph, k, mPathName,path_name_detail, old_node=None ):
        """
        Find all meta paths between two nodes within max_step.
        The loop stops only when k = 0.
        And the paths it find restore in self.map1.
        ...
        for neighbors in graph.neighbors(gene):
            ...
            if self.pruning(m_path_name2):
                continue
            self.save_map3(m_path_name2, neighbors,new_path_name_detail)
            if self.pruning2(m_path_name2):
                continue
            self.find_meta_paths_g3_center(neighbors, graph, k, m_path_name2, new_path_name_detail,old_node=gene)
    ...
    def save_map3(self, m_path_name2, neighbors, path_name_detail):
        if self.map3.__contains__(m_path_name2):
            if self.map3[m_path_name2].__contains__(neighbors):
                self.map3[m_path_name2][neighbors]['ins'] += 1
                self.map3[m_path_name2][neighbors]['list'].append(path_name_detail)

            else:
                self.map3[m_path_name2][neighbors] = {
                    'ins': 1,
                    'list': [path_name_detail]
                }
        else:
    ...
    def score(self,mPaths,mPathDict1,mPathDict2,mPathDict3,meta_paths_combine_list):
        """
            ...
            for name in i:
                for a in mPaths:
                    ...
                    if mPath==name:
                        ...
                        xmIns = self.get_ins_of_meta_path(meta_path_half, mPathDict3)
                        list_xmIns.append(xmIns)
            score=self.score_kernal_function(list_ins,list_hmIns,list_tmIns,list_xmIns)
            score_list.append({
                'mPath':total_name,
                'score':score,
            })
    ...
    def score_kernal_function(self,ins,hmIns,tmIns,xmIns):
        """
        The score function
        ...
        for l in xmIns:
            xmIns_total*=l
        score=3*ins_total/(hmIns_total+tmIns_total+xmIns_total)
        return score
