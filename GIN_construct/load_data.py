...
# tokenizer = BertTokenizer.from_pretrained('./PubMedBERT-base-uncased-abstract-fulltext')
    ...
    for j in range(len(index)-1):
        ...
        for line in segment:
            if(len(line.split())==1):
                continue
            lines = line.split("\t")
            word,bio_tag = lines[0],lines[-1]
            if(bio_tag[0]!='B' and bio_tag[0]!='I' and bio_tag!='O'):
                print(line)
            sent.append(word)
            for _ in bert_encode(word):  # 这里bert_encode实现可以用 tokenizer.tokenize()
                tag.append(bio_tag)
